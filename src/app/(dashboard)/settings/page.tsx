...
export default function SettingsPage() {
  ...
  // Sync form with user when not editing
  useEffect(() => {
    if (!editingProfile && user) {
      setProfileForm({ name: user.name || "", mobile: user.mobile || "", district: user.district || "" });
    }
  }, [user, editingProfile]);
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 transition-colors border-primary/30"
                />
              </div>
              <div>
